...
final public class WikiSorting {
    ...
    void MergeInPlace(int[] array, Range A, Range B) {
        ...
        /*
            ...
            when none of the A or B blocks in any subarray contained 2�닖A unique values,
            which places a hard limit on the number of times this will ACTUALLY need
            to binary search and rotate.

            according to my analysis the worst case is �닖A rotations performed on �닖A items
    ...
    void Sort(int[] array, int len) {
        ...
        while (true) {
            ...
            if (iterator.length() < cache_size) {
                ...
                // 1. pull out two internal buffers each containing �닖A unique values
                ...
                // we need to find either a single contiguous space containing 2�닖A unique values (which will be split up into two buffers of size �닖A each),
                // or we need to find one buffer of < 2�닖A unique values, and a second buffer of �닖A unique values,
                // OR if we couldn't find that many unique values, we need the largest possible buffer we can get

                // in the case where it couldn't find a single buffer of at least �닖A unique values,
                ...
                while (!iterator.finished()) {
                    ...
                    if (count >= buffer_size) {
                        ...
                        if (count == buffer_size + buffer_size) {
                            // we were able to find a single contiguous section containing 2�닖A unique values,
                            // so this section can be used to contain both of the internal buffers we'll need
                            buffer1.set(A.start, A.start + buffer_size);
                            buffer2.set(A.start + buffer_size, A.start + count);
                            break;
                        } else if (find == buffer_size + buffer_size) {
                            // we found a buffer that contains at least �닖A unique values, but did not contain the full 2�닖A unique values,
                            // so we still need to find a second separate buffer of at least �닖A unique values
                            ...
                            // we found a second buffer in an 'A' subarray containing �닖A unique values, so we're done!
                    ...
                    if (count >= buffer_size) {
                        ...
                        if (count == buffer_size + buffer_size) {
                            ...
                            // we found a second buffer in an 'B' subarray containing �닖A unique values, so we're done!
                ...
                while (!iterator.finished()) {
                    ...
                    if (start == pull[0].range.start) {
                        if (pull[0].from > pull[0].to) {
                            ...
                            // this only happens for very small subarrays, like �닖4 = 2, 2 * (2 internal buffers) = 4,
